...
function GuestRegistrationPage() {
  ...
  const sessionsQuery = useQuery({
    ...trpc.getPublicSessions.queryOptions({ eventId: selectedEventId || 0 }),
  ...
  // Helper function to safely find product selection index with error recovery
  const findProductSelectionIndex = useCallback((sessionIndex: number, productId: number, productTypeId: number) => {
    console.log(`Finding product selection index for session ${sessionIndex}, product ${productId}, type ${productTypeId}`);
    
    const session = sessionSelections?.[sessionIndex];
    if (!session) {
      console.error(`Session ${sessionIndex} not found in sessionSelections`);
      return -1;
    }
    
    if (!session.productSelections) {
      console.error(`Session ${sessionIndex} has no product selections array`);
      return -1;
    }
    
    const index = session.productSelections.findIndex(
      ps => ps.productId === productId && ps.productTypeId === productTypeId
    );
    
    if (index === -1) {
      console.warn(`Product selection not found for product ${productId}, type ${productTypeId} in session ${sessionIndex}`);
      console.log(`Available product selections in session ${sessionIndex}:`, session.productSelections);
      
      // Try to recover by re-initializing this specific product selection
      ...
      console.log(`Adding missing product selection:`, newProductSelection);
      
      // Add the missing product selection
      const updatedSelections = [...session.productSelections, newProductSelection];
      setValue(`sessionSelections.${sessionIndex}.productSelections`, updatedSelections);
      
      console.log(`Updated product selections for session ${sessionIndex}:`, updatedSelections);
      
      return updatedSelections.length - 1;
    }
    
    console.log(`Found product selection at index ${index}`);
    return index;
  }, [sessionSelections, setValue]);
  ...
  useEffect(() => {
    console.log("Session initialization effect triggered:", {
      sessionsDataLength: sessionsQuery.data?.length || 0,
      productsDataLength: productsQuery.data?.length || 0,
      sessionFieldsLength: sessionFields.length,
      isLoading: sessionsQuery.isLoading || productsQuery.isLoading
    });

    if (sessionsQuery.isLoading || productsQuery.isLoading) {
      console.log("Still loading data, skipping initialization");
      return;
    }

    if (!sessionsQuery.data || !productsQuery.data) {
      console.log("No session or product data available");
      return;
    }

    if (sessionsQuery.data.length === 0) {
      console.log("No sessions available for this event");
      return;
    }

    if (sessionFields.length > 0) {
      console.log("Session fields already initialized, skipping");
      return;
    }
    ...
    const newSessionSelections = sessionsQuery.data.map(session => {
      ...
      console.log(`Processing session ${session.id}: ${session.sessionName}`);
      console.log(`Session has ${session.productSessionMaps.length} product mappings`);
      
      session.productSessionMaps.forEach(psm => {
        console.log(`Processing product ${psm.product.id}: ${psm.product.productName} (${psm.product.productType})`);
        console.log(`Product has ${psm.product.productTypes.length} product types`);
        ...
        psm.product.productTypes.forEach(productType => {
          ...
          if (psm.product.productType === 'Entry') {
            ...
            console.log(`Auto-setting Entry quantity for ${productType.productSize}: ${quantity}`);
          } else {
            console.log(`Food product ${psm.product.productName} - ${productType.productSize}: quantity set to 0`);
          }
    ...
    console.log("Replacing session fields with new selections:", newSessionSelections);
    replaceSessionFields(newSessionSelections);
  }, [sessionsQuery.data, productsQuery.data, sessionFields.length, adults, children, elder, replaceSessionFields, calculateEntryQuantity, sessionsQuery.isLoading, productsQuery.isLoading]);
  ...
  useEffect(() => {
    if (sessionFields.length > 0 && productsQuery.data && sessionsQuery.data) {
      ...
      sessionFields.forEach((field, sessionIndex) => {
        ...
        // Get the session data to check product mappings
        const session = sessionsQuery.data.find(s => s.id === field.sessionId);
        if (!session) return;
        
        currentSession.productSelections.forEach((productSelection, productIndex) => {
          ...
          if (product?.productType === 'Entry' && productType) {
            ...
            if (currentSession.selected) {
              expectedQuantity = calculateEntryQuantity(productType.productSize, adults || 1, children || 0, elder || 0);
            }
            
            if (productSelection.quantity !== expectedQuantity) {
              ...
              setValue(`sessionSelections.${sessionIndex}.productSelections.${productIndex}.quantity`, expectedQuantity);
            }
          }
        });
      });
    }
  }, [adults, children, elder, sessionFields, sessionSelections, productsQuery.data, sessionsQuery.data, setValue, calculateEntryQuantity]);

  // Clear food quantities when opting out of food
  useEffect(() => {
    if (sessionFields.length > 0 && productsQuery.data && sessionsQuery.data) {
      sessionFields.forEach((field, sessionIndex) => {
        const currentSession = sessionSelections?.[sessionIndex];
        ...
        // If opted out of food, clear all food product quantities
        if (currentSession.optOutOfFood) {
          const session = sessionsQuery.data.find(s => s.id === field.sessionId);
          if (!session) return;
          ...
          currentSession.productSelections.forEach((productSelection, productIndex) => {
            ...
            if (product?.productType === 'Food' && productSelection.quantity > 0) {
              console.log(`Clearing food quantity for session ${sessionIndex}, product ${productIndex} due to opt-out`);
  ...
  }, [sessionSelections?.map(s => s.optOutOfFood), sessionFields, productsQuery.data, sessionsQuery.data, setValue]);
  ...
  const onSubmit = async (data: RegistrationForm) => {
    ...
    // Validate dine-in food selections
    let validationError = "";
    ...
    data.sessionSelections
      ...
      .forEach((session, sessionIndex) => {
        ...
        if (session.optOutOfFood) {
          console.log(`Session ${sessionIndex} opted out of food, skipping dine-in validation`);
          return;
        }
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-8">
            {process.env.NODE_ENV === 'development' && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                <h3 className="text-sm font-medium text-red-800 mb-2">🔧 Development Tools</h3>
                <div className="space-x-2">
                  <button
                    type="button"
                    onClick={() => {
                      console.log("Force refreshing session selections...");
                      replaceSessionFields([]);
                      setTimeout(() => {
                        if (sessionsQuery.data && productsQuery.data) {
                          const newSessionSelections = sessionsQuery.data.map(session => {
                            const productSelections: any[] = [];
                              ...
                              <div className="flex items-center">
                                ...
                                <div className="ml-3">
                                  <label className="text-sm font-medium text-yellow-900">
                                    Opt out of food for this session
                                  </label>
                                  <p className="text-xs text-yellow-700 mt-1">
                                    Check this if you don't want to order any food items for this session. You can still attend the event.
                                  </p>
                                </div>
                              </div>
                            </div>

                            {/* Food selection guidance - only show if not opted out */}
                              ...
                              .map((psm) => {
                                ...
                                return (
                                  <div key={product.id} className={`${isFood ? 'bg-green-50 border-green-200' : 'bg-gray-50'} border p-4 rounded-lg`}>
                                    ...
                                    {isFood ? (
                                      // Show food selection only if not opted out
                                      !sessionSelections?.[sessionIndex]?.optOutOfFood ? (
                                              ...
                                              <div key={personType} className="bg-white border-2 border-green-300 rounded-lg p-4">
                                                ...
                                                <div className="grid gap-3">
                                                  {availableOptions.map(productType => {
                                                    // Use the safer product selection index finding with error recovery
                                                    const productSelectionIndex = findProductSelectionIndex(sessionIndex, product.id, productType.id);
                                                    
                                                    // If still not found after recovery attempt, show error but don't crash
                                                    if (productSelectionIndex === -1) {
                                                      return (
                                                        <div key={productType.id} className="p-3 rounded-lg border bg-red-50 border-red-200">
                                                          <p className="text-red-600 text-sm">
                                                            Error: Product selection not found. Please refresh the page and try again.
                                                          </p>
                                                          <button
                                                            type="button"
                                                            onClick={() => window.location.reload()}
                                        ...
                                        // Show message when opted out of food
                                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                          <div className="flex items-center">
                                            <span className="text-yellow-600 text-lg mr-2">ℹ️</span>
                                            <p className="text-sm text-yellow-800">
                                              You have opted out of food for this session. Food selection is disabled.
                                            </p>
                                          </div>
                                        </div>
                                      )
              ...
              <>
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  ...
                  <div className="text-xs text-yellow-700 space-y-1">
                    ...
                    <div>Sessions Query Loading: {sessionsQuery.isLoading ? 'Yes' : 'No'}</div>
                    <div>Products Query Loading: {productsQuery.isLoading ? 'Yes' : 'No'}</div>
                    <div>Sessions Data Loaded: {sessionsQuery.data ? 'Yes' : 'No'} ({sessionsQuery.data?.length || 0} sessions)</div>
                    <div>Products Data Loaded: {productsQuery.data ? 'Yes' : 'No'} ({productsQuery.data?.length || 0} products)</div>
                    <div>Selected Event ID: {selectedEventId || 'None'}</div>
                    <div>Watched Event ID: {watchedEventId || 'None'}</div>
                    ...
                    <div>Adults: {adults}, Children: {children}, Elder: {elder}</div>
                    
                    <div className="mt-2 border-t pt-2">
                      <div className="font-medium">Session Details:</div>
                      {sessionSelections?.map((session, idx) => (
                        <div key={idx} className="ml-2 mt-1">
                          <div className="font-medium">Session {idx}: ID={session.sessionId}, Selected={session.selected ? 'Yes' : 'No'}, OptOut={session.optOutOfFood ? 'Yes' : 'No'}</div>
                          <div className="ml-2">Products: {session.productSelections?.length || 0}</div>
                          {session.productSelections?.map((ps, psIdx) => {
                            const product = productsQuery.data?.find(p => p.id === ps.productId);
                            const productType = product?.productTypes?.find(pt => pt.id === ps.productTypeId);
                            return (
                              <div key={psIdx} className="ml-4 text-xs">
                                {psIdx}: P{ps.productId}({product?.productName || 'Unknown'}) T{ps.productTypeId}({productType?.productSize || 'Unknown'}) Qty:{ps.quantity}
                              </div>
                            );
                          })}
                        </div>
                    ...
                    <div className="mt-2 border-t pt-2">
                      <div className="font-medium">Food Products Available:</div>
                      {productsQuery.data?.filter(p => p.productType === 'Food').map(product => (
                        <div key={product.id} className="ml-2 text-xs">
                          {product.productName} ({product.productTypes.length} types)
                          {product.productTypes.map(pt => (
                            <div key={pt.id} className="ml-4">
                              Type {pt.id}: {pt.productSize} - {pt.productChoice} - ${pt.productPrice} ({pt.productSubtype})
                            </div>
                          ))}
                        </div>
                      ))}
