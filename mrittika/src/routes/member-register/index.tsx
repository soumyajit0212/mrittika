...
function MemberRegistrationPage() {
        ...
        if (currentSession.optOutOfFood) {
          ...
          currentSession.productSelections.forEach((productSelection, productIndex) => {
            ...
            if (product?.productType === 'Food' && productSelection.quantity > 0) {
              console.log(`Member registration: Clearing food quantity for session ${sessionIndex}, product ${productIndex} due to opt-out`);
  ...
  // Calculate total cost (only for food items) - members get free entry
  const calculateTotalCost = () => {
    if (!sessionsQuery.data || !productsQuery.data) return 0;
    
    let total = 0;
    sessionSelections?.forEach((sessionSelection, sessionIndex) => {
      if (sessionSelection.selected && !sessionSelection.optOutOfFood) {
    ...
    data.sessionSelections
      ...
      .forEach((session, sessionIndex) => {
        ...
        if (session.optOutOfFood) {
          console.log(`Member registration: Session ${sessionIndex} opted out of food, skipping dine-in validation`);
          return;
        }
