...
function MemberRegistrationPage() {
  ...
  // Session toggle: keep Food at 0 by default; clear on deselect; ensure food visible
  const onToggleSession = (sessionIndex: number, selected: boolean) => {
    ...
    // Always set selected first so RHF updates synchronously
    setValue(
  ...
  // Submit with DINE-IN per-size equality check (must match person counts)
  ...
  // Helpers for DINE-IN group caps
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {sessionFields.map((field, sessionIndex) => {
                    ...
                    // Helper to find product/type by selection within this session
                    const findPT = (ps: any) => {
                              ...
                              <span className="flex items-center">
                                ...
                                {session.isFull ? (
                                  <span className="text-red-600 font-medium">
                                    Full ({session.sessionBalanceCapacity}/
                            ...
                            {!sessionSelections?.[sessionIndex]?.optOutOfFood && (
                              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                ...
                                <div className="text-sm text-blue-800 space-y-1">
                                  <p>
                                    <strong>Your Group:</strong> {adults} Adult{adults !== 1 ? "s" : ""},{" "}
                                    {children} Children, {elder} Elder{elder !== 1 ? "s" : ""}
                                  </p>
                                  <p>
                                    <strong>Dine-in meals:</strong> Select a total equal to your count per category
                                    (Adults/Children/Elders). Mixing across options is allowed but the sum cannot exceed
                                    the category total.
                                  </p>
                                  <p>
                                    <strong>Take-away items:</strong> Any quantity.
                                  </p>
                                </div>
                              </div>
                            )}
                                    ...
                                    {isFood && !sessionSelections?.[sessionIndex]?.optOutOfFood ? (
                                      ...
                                      <div className="space-y-5">
                                        {["Adult", "Children", "Elder"].map((personType) => {
                                          ...
                                          // Access all selections for this session once
                                            ...
                                            <div key={personType} className="bg-white border-2 border-green-300 rounded-lg p-4">
                                              ...
                                              <div className="grid gap-3">
                                                {availableOptions.map((pt: any) => {
                                                  ...
                                                  // Sum of other DINE-IN selections of the same size across the WHOLE session
                                                  ...
                                                  // This field can go up to the remaining allowance
                                                ...
                                                <div>
                                                  <label className="block text-sm font-medium text-gray-700">
                                                    ...
                                                    {product.productType === "Entry" ? (
                                                      ...
                                                      <span className="text-gray-600 ml-2">${pt.productPrice.toFixed(2)}</span>
                                                    )}
                                                  </label>
                                                </div>
